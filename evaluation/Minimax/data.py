...
class ImageFolder(datasets.DatasetFolder):
    ...
    def find_subclasses(self, nclass=100, phase=0, seed=0):
        ...
        if seed == 0:
            ...
            elif self.spec == 'cifa10':
                file_list = './misc/class_cifa10.txt'
            elif self.spec == 'imagenet1k':
                file_list = './misc/imagenet_1K.txt'
            else:
                file_list = './misc/class100.txt'
            print(f"class list file {file_list}")
...
def load_data(args, tsne=False):
    ...
    if args.dataset.startswith('cifar'):
        ...
        #     train_dataset = datasets.CIFAR10(args.data_dir, download=True,train=True, transform=train_transform)
        #     val_dataset = datasets.CIFAR10(args.data_dir, download=True,train=False, transform=test_transform)
        #     nclass = 10
        # else:
